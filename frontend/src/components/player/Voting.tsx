...
export function Voting() {
  ...
  // Dead players see a spectator screen
  if (isDead) {
    return (
      ...
      <div className="min-h-screen flex flex-col items-center justify-center bg-background p-6 gap-8">
        <div className="text-center space-y-3">
          ...
          <p className="text-gray-400 font-medium">
            You voted for: <span className="text-gray-900 font-bold">{selected === 'NO_VOTE' ? 'No Vote' : selected}</span>
          </p>
    ...
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <div className="flex-1 flex flex-col p-6">
        <StageHeader
          ...
          title="Vote for Mafia"
          subtitle="Who do you think is Mafia?"
        />
        ...
        <div className="space-y-3 mt-2">
          {alivePlayers.map(player => {
            ...
            return (
              ...
              >
                ...
                <span className={`font-bold text-lg ${selected === player.name ? 'text-primary' : 'text-gray-900'}`}>
                  {player.name}
                </span>
                {selected === player.name && (
          ...
          <button
            ...
            className={`w-full flex items-center justify-center gap-2 p-4 rounded-2xl border-2 text-center transition-all active:scale-[0.98] mt-2 ${
              selected === 'NO_VOTE'
